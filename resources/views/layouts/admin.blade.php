...
<body class="font-sans antialiased bg-brand-50" x-data="{ openSidebar: false }">
<div class="flex h-screen">

    <!-- Sidebar -->
    <div 
        class="bg-brand-800 text-brand-100 w-64 flex flex-col shadow-lg
               fixed inset-y-0 left-0 z-40 transform transition-transform duration-300
               -translate-x-full lg:translate-x-0 lg:relative lg:flex"
    ...
    >
        ...
        <nav class="flex-grow p-4 space-y-2">
            ...
            <a href="{{ route('admin.usuarios.index') }}" 
               class="flex items-center px-4 py-2 rounded-lg transition-colors duration-200 
                      {{ request()->routeIs('admin.usuarios.*') ? 'bg-brand-900 text-white font-semibold' : 'text-brand-200 hover:bg-brand-700 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.productos.index') }}"
               class="flex items-center px-4 py-2 rounded-lg transition-colors duration-200 
                      {{ request()->routeIs('admin.productos.*') ? 'bg-brand-900 text-white font-semibold' : 'text-brand-200 hover:bg-brand-700 hover:text-white' }}">
               <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.pedidos.index') }}"
               class="flex items-center px-4 py-2 rounded-lg transition-colors duration-200 
                      {{ request()->routeIs('admin.pedidos.*') ? 'bg-brand-900 text-white font-semibold' : 'text-brand-200 hover:bg-brand-700 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-4 border-t border-brand-700 space-y-2">
    ...
    <div class="flex-1 flex flex-col lg:ml-0 overflow-hidden">
        <header class="bg-white border-b border-brand-200 shadow-sm">
            <div class="px-6 py-4 flex justify-between items-center">
                <button class="lg:hidden mr-3" @click="openSidebar = true">
                    <svg class="w-7 h-7 text-brand-800" fill="none" stroke="currentColor" viewBox="0 0 24 24">
